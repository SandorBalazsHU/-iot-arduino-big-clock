...
byte leds2 = 0;    // Variable to hold the pattern of which LEDs are currently turned on or off
...
void loop() 
{
  ...
  {
    bitClear(leds2, i);    // Set the bit that controls that LED in the variable 'leds'
    updateShiftRegister();
    delay(500);
  }
   for (int i = 7; i >=0 ; i--) // Turn all the LEDs ON one by one.
